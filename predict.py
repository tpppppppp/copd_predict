...
def draw_force(model_best, X):
    ...
    print(explainer.expected_value, shap_values.values[0], X.iloc[0, :])
    shap_html = shap.force_plot(explainer.expected_value, shap_values.values[0], X.iloc[0, :], link='logit', matplotlib=False)
    return shap_html
